...
public class GenericUtils
{
	...
	public static <T> Class<?> getGenericType(Class<T> baseClass, Class<? extends T> childClass)
	{
		return (Class<?>) getTypeArguments(baseClass, childClass).get(0);
	}
		...
		else
		{
			...
			while (itr.hasNext())
			{
				...
				if (m.isVarArgs())
				{
					...
					// last parameter
					if (m.getParameterTypes().length != paramTypes.length || !m.getParameterTypes()[paramTypes.length - 1].isAssignableFrom(paramTypes[paramTypes.length - 1]))
						itr.remove();
				}
			}
